...
def compute_average_precision_detection(ground_truth, prediction, tiou_thresholds=np.linspace(0.5, 0.95, 10)):
    ...
    for idx, this_pred in prediction.iterrows():
        ...
        # 一个 GT 只能被匹配一次（被最高分的预测匹配），后续的算FP（重叠不够，位置不准;重复检测)
...
class ANETdetection(object):
    ...
    def _import_ground_truth(self, annotation_path):
        ...
        gtsegments = np.delete(gtsegments, [402, 226])  # 硬编码过滤，原有数据该样本标注错误
        ...
        videoname = np.array([i.decode("utf8") for i in videoname])  # 清洗脏数据
    ...
    def wrapper_compute_average_precision(self):
        """
        并行地为每一个类别调用 ‘compute_average_precision_detection’函数
        leverage joblib.Parallel for computing average precision for each class in the subset.
    ...
    def evaluate(self):
        ...
        self.mAP = self.ap.mean(axis=1)  # 对类别取平均，得到每个 tIoU 阈值下的 mAP
